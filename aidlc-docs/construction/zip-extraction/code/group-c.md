...
| 16 | `internal/bombdefence/checker.go` + `format.go` | `Checker.PreCheck` (rules #1, #4, #12), `OverlapCheck` (#11 Fifield defence), `EntryCheck` (#5, #6, #9), `NewLimitedReader` (#2, #3 short-circuiting). LimitedReader maintains sticky-error state — once tripped, subsequent reads return same error. Small-sample floor (64 KiB compressed) prevents false-positive ratio violations on tiny streams. |
